...
RUN (cd /opt/soapower && wget --no-check-certificate https://github.com/soapower/soapower/releases/download/2.0.0-RC2/soapower-2.0.0-RC2.zip -O soapower-2.0.0-RC2.zip)

# Unzipping Soapower
RUN (cd /opt/soapower && unzip soapower-2.0.0-RC2.zip)

# Create symbolic lynk
RUN (cd /opt/soapower && rm -f current; ln -s soapower-2.0.0-RC2 current)
